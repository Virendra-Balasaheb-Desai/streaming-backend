...
const toggleVideoLike = asyncHandler(async (req, res) => {
    ...
    if (!liked) {
        ...
        if (!removeLike) throw new ApiError(401, "Unable to remove like from video");
...
const toggleCommentLike = asyncHandler(async (req, res) => {
    ...
    if (!liked) {
        ...
        const removeLike = await Like.findByIdAndDelete(liked._id);
        if (!removeLike) throw new ApiError(401, "Unable to remove like from comment");
...
const toggleTweetLike = asyncHandler(async (req, res) => {
    ...
    if (!tweetId || !tweetId.trim()) throw new ApiError(401, "Unable to get tweet");
    ...
    if (!liked) {
        ...
        const removeLike = await Like.findByIdAndDelete(liked._id);
        if (!removeLike) throw new ApiError(401, "Unable to remove like from tweet");
